...
번역은 호출 비용보다 첫 결과의 품질이 중요한 작업이라 기본 모델을 `deepseek-v4-pro`로 설정했습니다. `전체 번역`을 한 번 누르면 브라우저가 곡 전체 가사를 문맥으로 제공하는 8줄 단위 API 요청을 순차적으로 새로 호출하고, 도착한 문장부터 화면과 Local Storage에 즉시 저장합니다. 각 서버 호출은 8.5초 안에 종료하며, 연결 시간 초과나 불완전한 응답은 브라우저가 같은 8줄을 별도 요청으로 최대 두 번 자동 재시도합니다. 그래도 실패하면 이미 받은 번역은 남고, `번역 이어하기`를 누르면 첫 미번역 줄부터 계속합니다. 이후 재생이나 재접속으로 번역 API를 다시 호출하지 않으며, 사용자가 이미 번역된 곡에서 `전체 번역`을 다시 눌러 덮어쓰기를 확인한 경우에만 처음부터 재호출합니다.
