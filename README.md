...
- 곡 전체 타이밍: `0.1초`/`0.5초 늦게·빠르게` 버튼으로 모든 문장을 함께 이동하고, 가운데 초 단위 값을 눌러 초기화
- 문장 시작점: 오른쪽에서 보정할 문장을 선택해 대상을 고정하고, 영상에서 실제 발화 시작점을 찾은 뒤 `이 문장을 현재 위치에 맞추기`로 보정

문장 시작점은 YouTube 플레이어의 시각을 버튼을 누르는 순간 직접 읽습니다. 보정 대상은 재생 추적과 별도로 고정되므로 영상을 움직이는 동안 다음 가사로 넘어가도 다른 문장이 수정되지 않습니다.
