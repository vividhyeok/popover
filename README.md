...
MV·라이브·가사 영상은 음원 버전과 인트로 길이가 다를 수 있습니다. 영상에서 첫 가사가 실제로 들리는 순간을 찾고 `현재 시점을 첫 가사 시작으로 지정` 버튼을 누르면 됩니다. Popover가 YouTube 플레이어의 현재 시각을 즉시 읽어 전체 가사를 같은 간격으로 이동합니다.

예를 들어 영상 13초에서 버튼을 누르면 첫 자막은 13초부터 표시되고 나머지 자막도 함께 이동합니다. 첫 자막의 실제 시작 전에는 해당 영어·한국어 문장을 미리 표시하지 않습니다. 보정 범위는 인트로가 긴 영상도 처리할 수 있도록 앞뒤 120초입니다.
