...
- 학습 행동은 영상 아래의 현재 문장 카드 한곳에 모읍니다. 각 어절을 별도 입력하고 `Space` 또는 `Enter`로 채점해 정답일 때 다음 어절로 이동합니다. 모든 어절을 맞혀도 자동으로 다음 문장으로 넘어가지 않으며, 사용자가 `Enter`를 한 번 더 눌러야 다음 학습 문장으로 이동합니다.
...
- 오른쪽 문장 목록은 재생 위치가 바뀌어도 자동 스크롤하지 않습니다. 사용자가 직접 스크롤하거나 문장을 클릭한 위치를 유지합니다.
